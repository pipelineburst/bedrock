...
class ApiGwStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, dict1, **kwargs) -> None:
        ...
        api_gateway = self.node.try_get_context('api_gateway')
        if api_gateway == "true":
            ...
            ### 1. Creating an api gateway that web applications can access to invoke the agent - this is the default behavior - the API will always be deployed 
            ...
            ### Optional resources based on user context variables

            ### 2. Create the ECS service for the Streamlit application if requested
            # OPTION: Was the "streamlit" variable passed in as part of the cdk deploy --context? 
            # If so then a Streamlit application will be created including supporing resources such as VPC, ALB, ECS...

            streamlit = self.node.try_get_context('streamlit')
            if streamlit:
                ...
                ### 3. Creating more optinoal resources based on user context variables 
